...
- [x] B: Add Mode 2 (generate from prompt)
  - [x] B.1: Add prompt-parsing and spec creation instructions — use `tinyspec new` for each feature
  - [x] B.2: Add auto-refine instructions — populate each spec with Background, Proposal, Implementation Plan, and Test Plan
  - [x] B.3: Add approval gate — present all generated specs to user before execution
- [x] C: Test the skill
  - [x] C.1: Run `tinyspec init` to ensure the new skill is registered
  - [x] C.2: Verify Mode 1 by invoking `/tinyspec:oneshot` with existing incomplete specs
  - [x] C.3: Verify Mode 2 by invoking `/tinyspec:oneshot` with a sample prompt
