...
Task groups represent logical units of work — typically a feature, a layer, or a phase. Subtasks break each group into individual steps. The `/tinyspec-work` skill implements groups in order, committing after each group completes.
...
4. **Implement** — `/tinyspec-work` or `/tinyspec-task` executes the plan
